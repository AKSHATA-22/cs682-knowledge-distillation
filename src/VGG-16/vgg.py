...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.droupout(self.activation(self.fc1(x)))
        x = self.droupout(self.activation(self.fc2(x)))
        x = self.fc3(x)
    ...
    def risk(self, Y, teacher_preds, output):
        student_output = torch.argmax(output, dim=1).to(torch.float32)
        return loss_l2(Y.to(torch.float32), student_output) + loss_kl_divergence(output, Y, teacher_preds, alpha=self.alpha, temperature=self.temperature)